...
- Every scene element needs an entrance tween; vary eases and avoid repeating one
  entrance pattern for all elements.
- Multi-scene edits require a visible transition. Do not pre-fade or slide out
  outgoing content before the transition; only the final scene may use exit
  fades.
